...
> **关于默认头像**：仓库里的 `webassets/ui/头像_妙妙.png` 和 `头像_嗔.png` 是我们两个人的形象，先借给大家当占位图～装好后把你们自己的头像图放进 `webassets/ui/`，在 `config.js` 里改一下文件名就换好啦。

> **退出键**：如果渔场是你家某个主页的子入口，在 `config.js` 里给 `exitUrl` 填上回程地址，左侧会出现一个退出键；留空则不显示。
